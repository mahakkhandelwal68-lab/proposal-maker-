 ...
 *
 * Not every deck's cover carries the same fields — `hasClientName: false`
 * skips the [Client Name] placeholder entirely, and `hasValidUntil: false`
 * fills the single [Date] with just the proposal date instead of expecting
 * a second [Date] for "valid until".
 *
...
async function fillTemplate({
  ...
  if (hasClientName) cover = cover.replace('[Client Name]', escapeXml(clientName));
  cover = cover.replace('[Company Name]', escapeXml(companyName));
  cover = replaceNthOccurrence(cover, '[Date]', escapeXml(proposalDate), 1);
  if (hasValidUntil) cover = replaceNthOccurrence(cover, '[Date]', escapeXml(validUntil), 1); // the remaining (2nd) [Date]
