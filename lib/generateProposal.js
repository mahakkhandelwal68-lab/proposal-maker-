...
// Uses the same local-time getters toLocaleDateString relies on internally,
// so this always matches the date formatDate() prints on the cover page —
// no risk of the two drifting a day apart across timezones.
...
// The name on the downloaded PDF, e.g.
// "Project_Proposal_Bhanu-Urja-Solar-Green-Energy_2026-09-22.pdf".
function buildDownloadFilename(companyName, date) {
  return `Project_Proposal_${sanitizeForFilename(companyName)}_${formatDateISO(date)}.pdf`;
}
...
async function generateProposal({ package: pkg, clientName, companyName, proposalDate, specificChanges, removePages, removePricing }, renderPdf) {
  ...
  // Scratch files on disk get their own unique name (a random id, so two
  // concurrent requests for the same company never collide) — separate
  // from the filename the employee actually sees on the downloaded PDF.
  const tmpDir = os.tmpdir();
  const id = crypto.randomBytes(6).toString('hex');
  const tempBase = `${sanitizeForFilename(companyName)}-${pkg}-${id}`;
  const pptxPath = path.join(tmpDir, `${tempBase}.pptx`);
  const pdfPath = path.join(tmpDir, `${tempBase}.pdf`);
  const trimmedTemplatePath = path.join(tmpDir, `${tempBase}-template.pptx`);
  ...
  return {
    pdfBuffer,
    filename: buildDownloadFilename(companyName.trim(), startDate),
    appliedEdits,
    unappliedEdits,
    geminiNote,
    geminiError,
